...
class ImagePatcher:
    def __init__(self, patch_size=224, overlap=0.5, bag_size=-1, empty_thresh=0.8):
        ...
        self.tiles = None
        logger.info(
            f"ImagePatcher initialized with patch_size={patch_size}, overlap={overlap}, bag_size={bag_size}, empty_thresh={empty_thresh}"
        )
    ...
    def reconstruct_attention_map(self, attention_weights, instances_ids, image_shape):
        ...
        reconstructed_attention = torch.zeros(
            (n_passes, n_classes, c, h, w), dtype=attention_weights.dtype, device=attention_weights.device
        )
