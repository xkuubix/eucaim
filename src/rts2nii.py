...
def create_mask(
    dcm: pydicom.FileDataset, dcm_rts: pydicom.FileDataset, ref_sop_uid: pydicom.uid.UID, bitwise_operator="OR"
) -> np.ndarray:
...
def plot_mask(dcm: pydicom.FileDataset, mask: np.ndarray) -> None:
...
def rtstruct2nii(path: str) -> None:
    """
                ...
                if filename.endswith(".dcm"):
                    ...
                    for ref_sop_uid in ref_sop_uid_mask_list:
                        if ref_sop_uid == dcm[SOP_Instance_UID].value:
                            seg_save_path = os.path.join(os.path.dirname(seg_path), f"segmentation_{ref_sop_uid}.nii.gz")
