...
with torch.no_grad():
    for batch in dataloaders["test"]:
        ...
        pred, patch_count = unet.patcher.reconstruct_image_from_patches(
            preds_patched, instances_ids, image_shape=images.shape
        )  # (c, h, w)
        mask, _ = (
            unet.patcher.reconstruct_image_from_patches(masks_patched, instances_ids, image_shape=images.shape)
            if masks is not None
            else (None, None)
        )
        att_weights_reconstructed, _ = (
            unet.patcher.reconstruct_image_from_patches(att_weights, instances_ids, image_shape=images.shape)
        ...
        fig.suptitle(
            f'Patch Size: {config["data"]["patch_size"]} (n={preds_patched.shape[0]}), Overlap: {config["data"]["overlap"]} | Image: {images.shape[-2]}x{images.shape[-1]}px',
            fontsize=16,
        )
        plt.tight_layout()
        plt.show()
