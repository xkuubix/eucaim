...
from mammography_tool.file_manipulation import make_long_format
    ...
    def __getitem__(self, idx):
        ...
        if annotation_path and os.path.exists(annotation_path):
            ...
            if self.dataframe.iloc[idx]["L1_radiological_assessment"] != "malign":
                annotation[annotation == 1.0] = 0.0
            if self.dataframe.iloc[idx]["L2_radiological_assessment"] != "malign":
                annotation[annotation == 2.0] = 0.0
            if self.dataframe.iloc[idx]["L3_radiological_assessment"] != "malign":
                annotation[annotation == 3.0] = 0.0

            annotation[annotation > 3.0] = 0.0  # 4 and 5 as 0
        ...
        sample["patientclass"] = torch.tensor(
            [1 if sample["classname"] == "malignant" else 0]
        ).long()  # Binary label for malignant vs non-malignant
        sample["record_id"] = self.dataframe.iloc[idx]["record_id"]
